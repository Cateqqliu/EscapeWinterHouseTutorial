...
2. 找到 **`Atlas Population Mode`** 這個選項。
3. 將它從預設的 `Static` (靜態) 改為 **`Dynamic` (動態)**！
4. 點擊右上角或滑到最下面按下 **`Apply`** 儲存設定。
...
> **🎯 為什麼 Dynamic 這麼強？**
> 開啟動態模式後，Unity 不會傻傻地把幾萬個中文字塞進遊戲裡。而是「玩家畫面上出現什麼字，系統才臨時去字型庫裡把那個字抓出來顯示」。這對需要大量文本與對話框的《Winter House》來說，絕對是節省效能與容量的終極必殺技！
